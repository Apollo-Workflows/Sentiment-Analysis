...
resource "aws_lambda_layer_version" "textblob_layer" {
  filename   = "../textblob-for-amazon-linux-env.zip"
...
resource "aws_lambda_layer_version" "tflite_layer" {
  filename   = "../tflite-for-amazon-linux-env.zip"
...
resource "aws_lambda_function" "lambda" {
  ...
  filename      = "${local.function_names[count.index]}.zip"
  ...
  source_code_hash = filebase64sha256( "${local.function_names[count.index]}.zip")
}
